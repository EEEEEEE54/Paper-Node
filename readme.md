...
### Windows error: `Identifier 'PORT' has already been declared`

If you see this on startup, your local `index.js` likely has two `const PORT ...` lines from a bad merge/edit.
This repo now uses `SERVER_PORT` internally to avoid that collision.
If your local file still errors, pull latest and make sure only one port declaration exists.
